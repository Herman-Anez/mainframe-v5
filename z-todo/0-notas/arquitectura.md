...
- **Adapter de salida** — implementa un port de salida. Acá: `InMemoryTodoListRepository`, `InMemoryEventBus`, `InMemoryUnitOfWork` (en `3-infrastructure/`).
...
- **Controller** — recibe una petición del mundo exterior y llama al use case correspondiente con el input armado. Acá: `TodoListController` (`3-backend-interface/`).
- **Presenter** — recibe el resultado de un use case (vía el Output Boundary) y decide cómo mostrarlo — consola, HTTP, UI. Acá: cada `*Presenter.ts` (`4-generic-implementation/`).
...
- **Frameworks & Drivers** (capa más externa) — bases de datos reales, frameworks web, UI — todo lo 100% intercambiable. Acá: `4-generic-implementation/` (consola), `4-angular/` (SPA real).
...
3-infrastructure       ← conoce la aplicación y el dominio, implementa lo que la app PIDE (ports de salida).
3-backend-interface    ← conoce la aplicación, expone un Controller genérico (adapter de entrada).
3-http-interface       ← conoce la aplicación, mapea casos de uso a rutas HTTP (otro adapter de entrada).
4-generic-implementation ← un pueblo: consola. Conoce todo, arma todo.
4-angular              ← otro pueblo: una SPA real. Conoce todo, arma todo a su manera.
```

`3-infrastructure`, `3-backend-interface` y `3-http-interface` son el mismo tier — ninguno es un "pueblo" completo, son kits reusables. Los pueblos (`4-*`) son los que efectivamente corren solos.

Esto no es una opinión ni un estilo — lo puedo **probar** con un comando:

```bash
grep -rln "3-infrastructure" core/1-domain core/2-application --include="*.ts"
# (sin resultados — ni domain ni application importan nada de infraestructura)
```

Cero resultados. Ni un solo archivo de `1-domain/` o `2-application/` (fuera de los tests, que son harness de prueba, no código de producción) importa algo de `3-infrastructure/`. Lo mismo vale para `3-backend-interface/` y `3-http-interface/` — ninguno de los dos importa `3-infrastructure` ni ningún `4-*` (salvo, de nuevo, tests usando los adapters in-memory como fakes). Las únicas carpetas que sí importan infraestructura y arman todo son `4-generic-implementation/main.ts` y `4-angular/` — los pueblos, armando el castillo cada uno a su manera.
...
`implements TodoListRepositoryPort` es literalmente "esta heladera tiene el enchufe correcto". Vive en `3-infrastructure/` — la carpeta de detalles técnicos.
...
- **Adapters de salida** — implementan un port de salida. `InMemoryTodoListRepository`, `InMemoryEventBus`, `InMemoryUnitOfWork`. Viven en `3-infrastructure/`.
- **Adapters de entrada** — usan un port de entrada para invocar la aplicación desde afuera. `TodoListController` es uno: no implementa nada, sino que **llama** a `useCase.execute(...)`. Vive en `3-backend-interface/` — mismo tier que `3-infrastructure`, dirección opuesta: ninguno de los dos es un frame completo, los dos son kits reusables que un frame (`4-generic-implementation`, `4-angular`) importa y completa.

Otro adapter de entrada, distinto: `3-http-interface/` — en vez de una clase con un método por caso de uso, describe cada endpoint como **datos** (`RouteDescriptor`: método, path, cómo armar el input) en vez de código imperativo. Mismo rol (adapter de entrada), forma distinta — ver Pieza 6 más abajo.
...
- **`TodoListController.ts`** (`3-backend-interface/`) — traduce "alguien llamó a `controller.addItem(listId, req, output)`" en "ejecutá el caso de uso `AddTodoItemUseCase` con este input". Recibe el `output` (presenter) como parámetro — no lo instancia, así no se casa con console.log ni con nada concreto.
- **Los `Presenter`** (`CreateTodoListPresenter`, etc, en `4-generic-implementation/api/presenters/`) — traducen "el caso de uso terminó con éxito/error" en "mostralo por consola" (podría ser "armá un JSON de respuesta HTTP", en otro contexto).
- **`RouteDescriptor`** (`3-http-interface/`) — mismo trabajo que el Controller, pero declarativo: cada `createXRoute(useCase)` describe qué HTTP endpoint corresponde a qué caso de uso, sin ejecutar nada por sí solo.
...
En este proyecto: `4-generic-implementation/main.ts` — el **composition root**. Es el único lugar del código que:
...
La carpeta se llama `4-generic-implementation` (y no `4-nest-implementation`, como se llamaba antes) justamente porque **no** compromete con ningún framework real — es el lugar donde iría Express/NestJS/Fastify si quisieras HTTP de verdad, pero hoy es solo wiring manual + un script que corre en consola.
...
**El problema, sin este truco**: normalmente, si `2-application/` necesita guardar una lista en una base de datos, tendría que importar directo el código de Postgres. Eso significa que `2-application` **depende de** `3-infrastructure`. Las flechas de dependencia apuntarían hacia afuera — justo lo que NO queremos (el círculo interno dependiendo del externo).

**El truco (Dependency Inversion Principle, la "D" de SOLID)**: en vez de que `2-application` importe el código real de Postgres, `2-application` **define una interfaz** (`TodoListRepositoryPort`) que dice "necesito algo que sepa hacer esto". Es `3-infrastructure` quien importa esa interfaz y la implementa (`implements TodoListRepositoryPort`).
...
```ts
// 3-infrastructure/persistence/InMemoryTodoListRepository.ts
...
1. TodoListController.addItem(listId, req, presenter)
     ← Interface Adapter (adapter de entrada, en 3-backend-interface, traduce la llamada)
...
8. AddTodoItemPresenter.presentSuccess(...)
     ← Interface Adapter (adapter de salida, en 4-generic-implementation, traduce a console.log)
```
...
| Interface Adapters | Adapters de entrada | `TodoListController` (`3-backend-interface/`), `RouteDescriptor`+rutas (`3-http-interface/`) |
| Interface Adapters | Adapters de salida | `*Presenter` (`4-generic-implementation/`), `InMemoryTodoListRepository`/`InMemoryEventBus`/`InMemoryUnitOfWork` (`3-infrastructure/`) |
| Frameworks & Drivers | El mundo exterior, fuera del hexágono | `4-generic-implementation/main.ts` (consola), `4-angular/` (SPA real) |
| Dependency Inversion Principle | (el mecanismo que hace posible los ports) | Interfaces en `2-application/ports/`, implementadas en `3-infrastructure/` |
